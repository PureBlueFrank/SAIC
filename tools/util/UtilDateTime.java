...
public class UtilDateTime {
    ...
    public static Timestamp addDaysToNow(Double days) {
        return new Timestamp(UtilDateTime.nowTimestamp().getTime() + ((long)(24L*60L*60L*1000L*days)));
    }
    ...
    public static java.sql.Timestamp formatStringToTimestamp(String dateTime){
    ...
    public static Timestamp getHourStart(Timestamp stamp){
    	return getHourStart(stamp, TimeZone.getDefault(), Locale.getDefault());
    }
    
    public static Timestamp getHourEnd(Timestamp stamp){
    	return getHourEnd(stamp, TimeZone.getDefault(), Locale.getDefault());
    }
    ...
    public static Timestamp getHourEnd(Timestamp stamp, TimeZone timeZone,Locale locale){
    	return getHourEnd(stamp, 0, timeZone, locale);
    }
    
    public static Timestamp getHourEnd(Timestamp stamp, int hoursLater, TimeZone timeZone,Locale locale){
    	Calendar tempCal = toCalendar(stamp, timeZone, locale);
    ...
    public static Timestamp getMonthEnd(Timestamp stamp) {
    	return getMonthEnd(stamp, 0, 0);
    }
    
    public static java.sql.Timestamp getMonthEnd(java.sql.Timestamp stamp, int daysLater, int monthsLater) {
        return getMonthEnd(stamp, daysLater, monthsLater, TimeZone.getDefault(), Locale.getDefault());
    }
    
    public static Timestamp getMonthEnd(Timestamp stamp, int daysLater, int monthsLater, TimeZone timeZone, Locale locale) {
        Calendar tempCal = toCalendar(stamp, timeZone, locale);
    ...
    /**
     * Localized Timestamp to String conversion. To be used in tandem with timeStampToString().
     */
    public static String timeStampToString(Timestamp stamp, String dateTimeFormat) {
        return timeStampToString(stamp, dateTimeFormat, Locale.getDefault());
    }
    
    /**
     * Localized Timestamp to String conversion. To be used in tandem with timeStampToString().
     */
    public static String timeStampToString(Timestamp stamp, String dateTimeFormat, Locale locale) {
        return timeStampToString(stamp, dateTimeFormat, TimeZone.getDefault(), locale);
    }
     ...
     * @return
     */
    public static String getDayBeforeMonth(int num){
    ...
    public static Timestamp getTimeStampBeforeMonth(int num){
    	...
    	ca.set(java.util.Calendar.HOUR, 0);
    	ca.set(java.util.Calendar.MILLISECOND, 0);
    	ca.add(Calendar.MONTH, - num); //月份减1   
    	return new java.sql.Timestamp(ca.getTimeInMillis());
    }
    
    //获取当前日期的前几天
    public static Timestamp getTimeStampBeforeDay(int num){
    	java.util.Calendar ca = java.util.Calendar.getInstance();//得到一个Calendar的实例   
    	ca.set(java.util.Calendar.SECOND, 0);
    	ca.set(java.util.Calendar.MINUTE, 0);
    	ca.set(java.util.Calendar.HOUR, 0);
    	...
    	ca.set(Calendar.DATE, day - num); 
    	return new java.sql.Timestamp(ca.getTimeInMillis());
    }
    
    // 将日期时间格式化后转化成相应的字符串
	public static String getDateTimeString(java.sql.Timestamp timeDate) {
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String dateTimeString = format.format(timeDate).toString();
		return dateTimeString;
	}
	
	// 将日期时间格式化后转化成相应的字符串
	public static String getDateTimeToString(java.sql.Timestamp timeDate) {
		SimpleDateFormat format = new SimpleDateFormat("yyyyMMddHHmmss");
		String dateTimeString = format.format(timeDate).toString();
		return dateTimeString;
	}
	// 将日期时间格式化后转化成相应的字符串
	public static String getDateTimeToString(java.sql.Timestamp timeDate,String dateformat) {
		SimpleDateFormat format = new SimpleDateFormat(dateformat);
		String dateTimeString = format.format(timeDate).toString();
		return dateTimeString;
	}
	
	public static String getDetailDateTimeToString(java.sql.Timestamp timeDate) {
	...
	// 将日期时间格式化后转化成相应的微秒数
	public static Long getDateTimeNum(java.sql.Timestamp timeDate) {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
		String dateTimeString = sdf.format(timeDate).toString();
		long time = 0L;
		try {
			time = sdf.parse(dateTimeString).getTime();
		} catch (ParseException e) {
	...
	// 计算两个日期之间的时间间隔
	public static List<Integer> getIntervalTime(Timestamp thruDate) {
		if(thruDate != null) {
			// 获取活动结束时间
			String promotionTime = getDateTimeString(thruDate);
			// 获取现在的时间
			String timeData = UtilDateTime.nowDateString();
			long endDate = 0, startDate = 0, intervalDay = 0, intervalHour = 0, intervalMinute = 0, intervalSecond = 0;
			endDate = getDateTimeNum(java.sql.Timestamp.valueOf(promotionTime));
			startDate = getDateTimeNum(java.sql.Timestamp.valueOf(timeData));
			intervalDay = (endDate - startDate) / (1000 * 60 * 60 * 24);    //date3为date1与date2之间的天数
			intervalHour = ((endDate - startDate) - ((1000 * 60 * 60 * 24) * intervalDay)) / (1000 * 60 * 60);
			intervalMinute = ((endDate - startDate) - ((1000 * 60 * 60 * 24) * intervalDay) - (intervalHour * (1000 * 60 * 60))) / (1000 * 60);
			intervalSecond = ((endDate - startDate) - ((1000 * 60 * 60 * 24) * intervalDay) - (intervalHour * (1000 * 60 * 60)) - (intervalMinute * (1000 * 60))) / 1000;
			
			List<Integer> list = new ArrayList<Integer>();
			list.add(new Long(intervalDay).intValue());
			list.add(new Long(intervalHour).intValue());
			list.add(new Long(intervalMinute).intValue());
			list.add(new Long(intervalSecond).intValue());
			return list;
		} else {
			List<Integer> list = new ArrayList<Integer>();
	...
	public static int getListValue(List<Integer> list, int index) {
		int num = list.get(index).intValue();
		return num;
	}
    ...
    public static String getTimeBeforeMonth(int month) {
    	Calendar ca = Calendar.getInstance();	//得到一个Calendar的实例   
        ca.add(Calendar.MONTH, - month);		//月份减6     
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");  
        String time = sdf.format(ca.getTime());
        return time;
    }
	...
	public static String getDaysBeforeNow(Date date) {
		...
		ca1.setTime(date);
		String strDay = ca.get(java.util.Calendar.DAY_OF_YEAR) - ca1.get(java.util.Calendar.DAY_OF_YEAR) + "天 ";
		String strHour = ca.get(java.util.Calendar.HOUR_OF_DAY) - ca1.get(java.util.Calendar.HOUR_OF_DAY) + "小时 ";
		String strMinute = ca.get(java.util.Calendar.MINUTE) - ca1.get(java.util.Calendar.MINUTE) + "分钟 ";
		String strSec = ca.get(java.util.Calendar.SECOND) - ca1.get(java.util.Calendar.SECOND) + "秒 ";
		return  strDay + strHour + strMinute + strSec;
	}
	/**
	 * 计算天数间隔
	 */
	public static int getDaysBeforeNowDate(Date date) {
		if(date == null) return 0;
		java.util.Calendar ca = java.util.Calendar.getInstance();
		ca.setTime(new Date());
		java.util.Calendar ca1 = java.util.Calendar.getInstance();
		ca1.setTime(date);
		int strDay = ca1.get(java.util.Calendar.DAY_OF_YEAR) - ca.get(java.util.Calendar.DAY_OF_YEAR);
		return  strDay >=0 ? strDay : 0;
	}
	 ...
	 * @param args
	 */
	public static String getDaysAfterNow(int num){
		
	
	    	Calendar ca = Calendar.getInstance();//得到一个Calendar的实例   
	        ca.add(Calendar.YEAR, +num); //月份减1     
	        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");  
	        String time = sdf.format(ca.getTime());
	        return time;
	 ...
	 * @param args
	 */
	public static String getDaysAfterMonth(int num){
	 ...
	 * @param args
	 */
	public static String getWeeksAfterMonth(int num1,int num2){		
	
	    	Calendar ca = Calendar.getInstance();//得到一个Calendar的实例   
	    	ca.add(Calendar.MONTH, +num1);
	        ca.add(Calendar.DATE, -num2); //月份减1     
	        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");  
	        String time = sdf.format(ca.getTime());
	        return time;
	 ...
	 * @param args
	 */
	public static String getWeeksAfterNow(int num1,int num2){
		
	
	    	Calendar ca = Calendar.getInstance();//得到一个Calendar的实例   
	    	ca.add(Calendar.YEAR, +num1);
	        ca.add(Calendar.DATE, -num2); //月份减1     
	        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");  
	        String time = sdf.format(ca.getTime());
	        return time;
