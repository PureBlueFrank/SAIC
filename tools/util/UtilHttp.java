        ...
        while (e.hasMoreElements()) {
            ...
            if (paramArr != null) {
                if (paramArr.length > 1) {
                	...
                	//by chengj for tomcat param value error encoding in UTF-8 
                    if("GET".equalsIgnoreCase(request.getMethod())){
                    	 try {
							value = new String(paramArr[0].getBytes("ISO-8859-1"),"UTF-8");
...
//        Debug.logInfo("CQ stripNamedParamsFromQueryString============"+retStr, module);
        return retStr;
    }
    ...
    public static final String EMAR_SRC = "emar";
    public static final String EMAR_SRC_COOKIE_NAME = "EMAR_SRC";
    public static final String EMAR_CHANNEL_COOKIE_NAME = "EMAR_CHANNEL";
    public static final String EMAR_CID_COOKIE_NAME = "EMAR_CID";
    public static final String EMAR_WI_COOKIE_NAME = "EMAR_WI";
    public static String[] emarCookieNames = new String[]{EMAR_SRC_COOKIE_NAME,EMAR_CHANNEL_COOKIE_NAME,EMAR_CID_COOKIE_NAME,EMAR_WI_COOKIE_NAME};
    public static String emarPreUrl = "http://o.yiqifa.com/servlet/handleCpsIn?";
     ...
     * @return boolean
     */
    public static Map<String, String> getLocalEmarCookie(HttpServletRequest request) {
        Cookie[] cookies = request.getCookies();
        Map<String, String> cookieMap = null;
        if(cookies != null) {
            cookieMap = FastMap.newInstance();
            for(int i =0; i<emarCookieNames.length; i++) {
                for(int j =0; j<cookies.length; j++) {
                    if(emarCookieNames[i].equals(cookies[j].getName())) {
                        cookieMap.put(emarCookieNames[i], cookies[j].getValue());
                    }
                }
            }
            if(cookieMap.size() != emarCookieNames.length) {
                return null;
            }
        }
        return cookieMap;
    ...
    public static void addEmarVisitorCookie(Map<String, String> requestMap, HttpServletResponse response) {
        Cookie srcCookie = new Cookie(EMAR_SRC_COOKIE_NAME, requestMap.get("src"));
        Cookie channelCookie = new Cookie(EMAR_CHANNEL_COOKIE_NAME, requestMap.get("channel"));
        Cookie cidCookie = new Cookie(EMAR_CID_COOKIE_NAME, requestMap.get("cid"));
        Cookie wiCookie = new Cookie(EMAR_WI_COOKIE_NAME, requestMap.get("wi"));
        String domain = UtilProperties.getPropertyValue("url.properties", "cookie.domain");
        int age = 60 * 60 * 24 * 30;
        wiCookie.setMaxAge(age);
        wiCookie.setPath("/");
        wiCookie.setDomain(domain);
        
        srcCookie.setMaxAge(age);
        srcCookie.setPath("/");
        srcCookie.setDomain(domain);
        
        cidCookie.setMaxAge(age);
        cidCookie.setPath("/");
        cidCookie.setDomain(domain);
        
        channelCookie.setMaxAge(age);
        channelCookie.setPath("/");
        channelCookie.setDomain(domain);
    ...
    public static String getUTF8_EncodeFields(String filed) {
        try {
            return URLEncoder.encode(filed, "UTF-8");
        } catch (UnsupportedEncodingException e) {
            Debug.logError("转换编码时报错" , module);
            return"";
        }
    }    
    ...
    public static boolean isInnerIP(String ipAddress){
    	if("0:0:0:0:0:0:0:1".equals(ipAddress)){	//localhost
    		return true;
    	}
