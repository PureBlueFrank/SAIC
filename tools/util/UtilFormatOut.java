...
public class UtilFormatOut {
    ...
    public static String formatCurrency(double price, String isoCode, Locale locale, int maximumFractionDigits) {
        ...
        String pstr = nf.format(price);
        pstr = pstr.replaceAll(",", "");
//        if(pstr.indexOf("￥") != -1){
//        	pstr = parseCurrencyToChn(pstr);
//        }
        return pstr;
    }
    
    public static String parseCurrencyToChn(String pstr){
    	pstr = pstr.replaceAll(",", "");
		if(pstr.indexOf(".") != -1){
			String decimal = pstr.substring(pstr.indexOf(".") + 1);
			decimal = skipLastZero(decimal);
	    	String number = pstr.substring(0, pstr.indexOf("."));
    ...
    public static String skipLastZero(String decimal){
     ...
     * @return
     */
    public static String formatAttributeValuesString(String attributeValues) {
    	String str = "";
    	if (UtilValidate.isNotEmpty(attributeValues)) {
    		int flag = -1;
    		String[] array = attributeValues.split("/");
    		for (int i=0; i<array.length; i++) {
    			String[] index = array[i].split("_");
    			for (int j=i+1; j<array.length; j++) {
    				String[] temp = array[j].split("_");
    				if (index[1].equals(temp[1])) {
    					flag = 0;
    					break;
    				} else {
    					flag = 1;
    		...
    		if ("".equals(str)) {
    			...
    			str = array[array.length-1] + "/" + str;
    		}
    		attributeValues = str;
    		str = "";
    		// println("attributeValues = " + attributeValues);
    	}
     ...
     * @return
     */
    public static String formatAttributeValues(String attributeValues) {
    	String str = "";
    	if (UtilValidate.isNotEmpty(attributeValues)) {
    		int flag = -1;
    		String[] array = attributeValues.split("\\|");
